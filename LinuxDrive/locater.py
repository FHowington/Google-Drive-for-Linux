...
class Locater:
    ...
    def find(self, full_path, previous_folder):
        ...
        # See if the last element in the path delimited can be found in the current folder, thereby short-circuiting it
        page_token = None
        folder_located = False
        # Better yet, see if last folder actually IS the current folder!!
        # New idea. Keep track of last path that was searched and base decision on that. Much safer.

        response = self.drive.service.files().list(q="mimeType='application/vnd.google-apps.folder'"
                                                     and "'%s' in parents" % previous_folder
                                                     and "name='%s'" % path_delimited[-1],
                                                   spaces='drive',
        ...
        for folder in response.get('files', []):
            ...
            if folder.get('name') == path_delimited[-1] and previous_folder in folder.get('parents'):
                folder_id = folder.get('id')
                folder_located = True
                print("Successfully short circuited")
                break
